...
void replace_all_placeholders( std::string& ua_property, std::smatch result, std::map<std::string::size_type, size_t> replacement_map) {
...
int main( int argc, char ** argv ){
  std::ifstream regexesFile;
  regexesFile.exceptions( std::ifstream::badbit | std::ifstream::failbit);
  regexesFile.open("regex.list");
  std::string regexValue;
  while( !regexesFile.eof() ){
    std::getline(regexesFile, regexValue);
    break;
  }
  try {
    std::regex t(regexValue);
  }catch (std::regex_error& e) {
    print_regexp_error(regexValue,e);
  }
  return 0;
}
