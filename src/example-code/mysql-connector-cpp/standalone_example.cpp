...
*
/usr/bin/g++ -o standalone -I/usr/local/include/cppconn/ -Wall -lmysqlcppconn examples/standalone_example.cpp
*
...
{
  string host(argc >= 2 ? argv[1] : EXAMPLE_HOST);
  const string user(argc >= 3 ? argv[2] : EXAMPLE_USER);
  const string pass(argc >= 4 ? argv[3] : EXAMPLE_PASS);
  const string database(argc >= 5 ? argv[4] : EXAMPLE_DB);
  const string port( argv[5] );
  const string url = "tcp://"+host+":"+port;
  cout << endl;
  ...
  try {
    ...
    std::cout<<"mysql_url: "<<url<<" username: "<<user<<" password: "<<pass<<"\n";
