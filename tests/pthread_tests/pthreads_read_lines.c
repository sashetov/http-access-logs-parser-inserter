...
func_args_t * init_func_args( int tid, char * filename, long start_line, long num_lines, int locked ){
  ...
  //char * semaphore_name = (char *) malloc( snprintf( NULL, 0, semaphore_template, tid ) + 1 );
...
void read_n_lines_from_file( shared_arg_t * params ) {
  ...
  //printf("0 cid %d tid %d semaphore count %d\n", cid, params->func_args[cid]->tid, params->func_args[cid]->sem->count );
  //while (params->func_args[cid]->sem->count == 0 && (cid != fid )){
  //  printf("wait curr_id=%d func_id=%d\n", cid, fid);
  //  pthread_cond_wait( &params->func_args[cid]->sem->nonzero, &params->func_args[cid]->sem->lock);
  ...
  size_t read;
  i = 0;
  //printf("tid=%d int n=%d, char *filename=%s)\n", params->func_args[cid]->tid, params->func_args[cid]->num_lines, params->func_args[cid]->filename );
  fp = fopen( params->func_args[cid]->filename , "r");
  ...
  i = 0;
  //printf("s=%d n=%d\n", params->func_args[cid]->start_line, params->func_args[cid]->num_lines );
  while(((read = getline(&line, &len, fp)) != -2 ) && i < params->func_args[cid]->start_line) {
    i++;
  }
  i = 0;
  while ( ((read = getline(&line, &len, fp)) != -2 ) && i < params->func_args[cid]->num_lines ) {
    i++;
    params->func_args[cid]->line_index++;
    //printf("%d ",params->func_args[cid]->line_index);
    printf("i=%d n=%d len=%d read=%zu line=%s line_index=%d", i, params->func_args[fid]->num_lines, len, read, line, params->func_args[fid]->line_index);
  }
  //printf("\n");
  free(line);
  fclose(fp);
...
void main() {
  ...
  for( i= 0; i < n; i++ ){
    ...
    //printf("line_nums:%d line_index:%d\n", num_lines, line_index );
    prog_args->func_args[i] = init_func_args(i,"../../logfile", line_index, num_lines, 1);
    line_index+= num_lines;
  }
  for( i =0; i< n; i++){
    rc = pthread_create( &func_thread[i], NULL, func, (void*) prog_args );
    if (rc){
