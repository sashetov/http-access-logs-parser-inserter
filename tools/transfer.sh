...
function __main__ {
  ...
  for HOSTNUM in 001 002 003; do {
    ...
    printdo "ssh $SSH_URL 'mkdir -p ${REMOTE_BASEDIR}/{lib,bin,sql}'";
    ...
    while read LIBPATH; do {
      ...
      printdo "scp $LIBPATH $SSH_URL:$REMOTE_BASEDIR/lib/";
    ...
    printdo "scp cloudstats $SSH_URL:$REMOTE_BASEDIR/bin/;";
    printdo "scp uap_regexes.yaml $SSH_URL:$REMOTE_BASEDIR/bin/;";
    printdo "scp search_engines $SSH_URL:$REMOTE_BASEDIR/bin/;";
    printdo "scp src/sql/schemas/*.sql $SSH_URL:$REMOTE_BASEDIR/sql/";
    printdo "scp *.mmdb $SSH_URL:$REMOTE_BASEDIR/bin/";
  }; done;
}
__main__ $*
