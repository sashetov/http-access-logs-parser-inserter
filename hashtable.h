...
//@brief Set the callback which must be called to release values stored in the table @param table : A valid pointer to an hashtable_t structure @param cb : an ht_free_item_callback_t function
void ht_set_free_item_callback(hashtable_t *table, ht_free_item_callback_t cb);
//@brief Clear the table by removing all the stored items @param table : A valid pointer to an hashtable_t structure If a free_item_callback has been set, that will be called for each item removed from the table 
void ht_clear(hashtable_t *table);
//@brief Destroy the table by releasing all its resources @param table : A valid pointer to an hashtable_t structure If a free_item_callback has been set, that will be called for each item removed from the table
void ht_destroy(hashtable_t *table);
//@brief Get the value stored at a specific key @param table : A valid pointer to an hashtable_t structure @param key   : The key to use @param klen  : The length of the key @param dlen  : If not NULL, the size of the returned data will be stored at the address pointed by dlen @return The stored value if any, NULL otherwise 
...
// @brief Callback for the key iterator @param table : A valid pointer to an hashtable_t structure @param key : The key @param klen : The length of the key @param user : The user pointer passed as argument to the ht_foreach_pair() function @return HT_ITERATOR_CONTINUE to go ahead with the iteration, HT_ITERATOR_STOP to stop the iteration, HT_ITERATOR_REMOVE to remove the current item from the table and go ahead with the iteration HT_ITERATOR_REMOVE_AND_STOP to remove the current item from the table and stop the iteration
typedef ht_iterator_status_t (*ht_key_iterator_callback_t)(hashtable_t *table, void *key, size_t klen, void *user);
//@brief Key iterator @param table : A valid pointer to an hashtable_t structure @param cb    : an ht_key_iterator_callback_t function @param user  : A pointer which will be passed to the iterator callback at each call
void ht_foreach_key(hashtable_t *table, ht_key_iterator_callback_t cb, void *user);
//@brief Callback for the value iterator @param table : A valid pointer to an hashtable_t structure @param value : The value @param vlen : The length of the value @param user : The user pointer passed as argument to the ht_foreach_pair() function @return HT_ITERATOR_CONTINUE to go ahead with the iteration, HT_ITERATOR_STOP to stop the iteration, HT_ITERATOR_REMOVE to remove the current item from the table and go ahead with the iteration HT_ITERATOR_REMOVE_AND_STOP to remove the current item from the table and stop the iteration
typedef ht_iterator_status_t (*ht_value_iterator_callback_t)(hashtable_t *table, void *value, size_t vlen, void *user);
// @brief Value iterator @param table : A valid pointer to an hashtable_t structure @param cb    : an ht_value_iterator_callback_t function @param user  : A pointer which will be passed to the iterator callback at each call 
void ht_foreach_value(hashtable_t *table, ht_value_iterator_callback_t cb, void *user);
// @brief Callback for the pair iterator @param table : A valid pointer to an hashtable_t structure @param key : The key @param klen : The length of the key @param value : The value @param vlen : The length of the value @param user : The user pointer passed as argument to the ht_foreach_pair() function @return HT_ITERATOR_CONTINUE to go ahead with the iteration, HT_ITERATOR_STOP to stop the iteration, HT_ITERATOR_REMOVE to remove the current item from the table and go ahead with the iteration HT_ITERATOR_REMOVE_AND_STOP to remove the current item from the table and stop the iteration
...
//@brief Pair iterator @param table : A valid pointer to an hashtable_t structure @param cb    : an ht_pair_iterator_callback_t function @param user  : A pointer which will be passed to the iterator callback at each call
