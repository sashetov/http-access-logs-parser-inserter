...
int ht_init(hashtable_t *table, size_t initial_size, size_t max_size, ht_free_item_callback_t cb) {
  ...
  TAILQ_FOREACH_SAFE(list, &table->iterator_list->head, iterator_next, tmp) {
    ...
    while((item = TAILQ_FIRST(&list->head))) {
      ...
      if (!new_list) {
        ...
        // XXX - if malloc fails here the table is irremediably corrupted so there is no point in handling the case.
        //TODO : using an internal prealloc'd bufferpool would ensure us to always obtain a valid pointer here
...
static inline ht_items_list_t * ht_get_list(hashtable_t *table, uint32_t hash) {
  size_t index = hash%ATOMIC_READ(table->size);
  // first try updating the status assuming we are the first reader requesting access to the table
  uint32_t status;
  do {
    status = ATOMIC_CAS_RETURN(table->status, HT_STATUS_IDLE, HT_STATUS_READ);
    // NOTE : if some writer is accessing the table we need to wait, multiple readers (status greater than IDLE) are allowed.  In the unlikely event that sched_yield() fails, we break the loop but we will still check if the status is valid and in case it's not (so the cas operation didn't succeed) we will synchronize again with the other threads
  } while (status < HT_STATUS_IDLE && sched_yield() == 0);
  // if some other reader is running in a background thread and has already updated the status (so it's already greater than IDLE), let's take that into account and try incrementing the status value by one
  while (status != HT_STATUS_IDLE && !(status >= HT_STATUS_READ && ATOMIC_CAS(table->status, status, status + 1))) {
    // if we didn't succeed incrementing the status, maybe the other readers finished their job and it was already put back to IDLE
    status = ATOMIC_CAS_RETURN(table->status, HT_STATUS_IDLE, HT_STATUS_READ);
    // some writer is accessing the table, we need to wait
    if (status < HT_STATUS_IDLE)
      sched_yield();
  }
  // status now holds the value we have updated in table->status
  status++;
  // we can now safely retrieve the list
  ht_items_list_t *list = table->items[index];
  // NOTE: it's important here to lock the list while the status has not been put back to idle yet (so writes can't happen)
  if (list)
    SPIN_LOCK(list->lock);
  // now let's update the status by decrementing it NOTE: if we are the last active reader it will go down to the idle state
  do {
...
static inline ht_items_list_t * ht_set_list(hashtable_t *table, uint32_t hash) {
  ...
  if (table->items[index]) {
    // if there is a list already set at our index it means that some other thread succeded in setting a new list already, completing its job before we were able to update the table status.  So we can release our newly created list and return the existing one
    SPIN_UNLOCK(list->lock);
    SPIN_DESTROY(list->lock);
    free(list);
  ...
  // it's safe to assume the status still WRITE, so we don't need to check if the CAS operation succeeded
  ATOMIC_CAS(table->status, HT_STATUS_WRITE, HT_STATUS_IDLE);
  MUTEX_LOCK(table->iterator_lock);
...
static inline int ht_set_internal(hashtable_t *table, void *key, size_t klen, void *data, size_t dlen, void **prev_data, size_t *prev_len, int copy, int inx) {
  ...
  if (ht_count(table) > (current_size + (current_size/3)) && (!table->max_size || current_size < table->max_size)) {
    ht_grow_table(table);
  }
  if (prev) {
    if (prev_data)
...
static int ht_set_if_equals_helper(hashtable_t *table, void *key __attribute__ ((unused)), size_t klen __attribute__ ((unused)), void **value, size_t *vlen, void *user) {
  ht_set_if_equals_helper_arg_t *arg = (ht_set_if_equals_helper_arg_t *)user;
  if (arg->prev_len)
    *arg->prev_len = *vlen;
  if (arg->prev_data)
    *arg->prev_data = *value;
  if (arg->match_size == *vlen && ((char *)*value)[0] == *((char *)arg->match) && memcmp(*value, arg->match, arg->match_size) == 0) {
    arg->matched = 1;
...
static int ht_delete_helper(hashtable_t *table, void *key __attribute__ ((unused)), size_t klen __attribute__ ((unused)), void **value, size_t *vlen, void *user) {
...
static inline int ht_delete_internal (hashtable_t *table, void *key, size_t klen, void **prev_data, size_t *prev_len, void *match, size_t match_size) {
  ht_delete_helper_arg_t arg = {
    .unset = 0,
    .prev_data = prev_data,
    .prev_len = prev_len,
...
static int ht_foreach_key_helper(hashtable_t *table, void *key, size_t klen, void *value __attribute__ ((unused)), size_t vlen __attribute__ ((unused)), void *user) {
...
static int ht_foreach_value_helper(hashtable_t *table, void *key __attribute__ ((unused)), size_t klen __attribute__ ((unused)), void *value, size_t vlen, void *user) {
  ht_iterator_arg_t *arg = (ht_iterator_arg_t *)user;
