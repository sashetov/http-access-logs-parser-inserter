...
int list_init(linked_list_t *list __attribute__ ((unused))) {
#ifdef THREAD_SAFE
...
size_t list_count(linked_list_t *l) {
  size_t len;
  MUTEX_LOCK(l->lock);
  len = l->length;
  MUTEX_UNLOCK(l->lock);
...
void list_lock(linked_list_t *list __attribute__ ((unused))) {
  MUTEX_LOCK(list->lock);
}
void list_unlock(linked_list_t *list __attribute__ ((unused))) { MUTEX_UNLOCK(list->lock);
}
static inline list_entry_t *create_entry() {
...
static inline int unshift_entry(linked_list_t *list, list_entry_t *entry) { list_entry_t *p;
  if(!entry)
    return -1;
  MUTEX_LOCK(list->lock);
  if(list->length == 0)
...
void * list_pick_value(linked_list_t *list, size_t pos) { list_entry_t *entry = pick_entry(list, pos);
  if(entry)
    return entry->value;
  return NULL;
}
...
void * list_set_value(linked_list_t *list, size_t pos, void *newval) {
  void *old_value = NULL;
  MUTEX_LOCK(list->lock);
  list_entry_t *entry = pick_entry(list, pos);
  if (entry) {
...
void * list_subst_value(linked_list_t *list, size_t pos, void *newval) {
  void *old_value = NULL;
  MUTEX_LOCK(list->lock);
  list_entry_t *entry = pick_entry(list, pos);
  if (entry) {
...
int list_foreach_value(linked_list_t *list, int (*item_handler)(void *item, size_t idx, void *user), void *user) {
  MUTEX_LOCK(list->lock);
  slice_t slice = {
    .list = list,
    .offset = 0,
...
tagged_value_t * list_create_tagged_value(char *tag, void *val, size_t vlen) {
  tagged_value_t *newval = (tagged_value_t *)calloc(1, sizeof(tagged_value_t));
  if(!newval) {
    //fprintf(stderr, "Can't create new tagged value: %s", strerror(errno));
    return NULL;
...
static inline void list_quick_sort(list_entry_t *head, list_entry_t *tail, list_entry_t *pivot, int length, list_comparator_callback_t comparator) {
...
int slice_foreach_value(slice_t *slice, int (*item_handler)(void *item, size_t idx, void *user), void *user) {
  linked_list_t *list = slice->list;
  MUTEX_LOCK(list->lock);
  size_t idx = 0;
